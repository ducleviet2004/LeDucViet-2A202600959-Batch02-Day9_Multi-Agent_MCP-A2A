...
def generate_with_citation(query: str, top_k: int = TOP_K, use_reranking: bool = True) -> dict:
    ...
    Args:
        ...
        top_k: Số lượng chunks lấy về
        use_reranking: Có sử dụng reranker không
    ...
    # 1. Thử sử dụng Gemini nếu có cấu hình key
    ...
    # 2. Sử dụng OpenAI nếu có key hợp lệ và không rỗng
    if openai_key and openai_key != "sk-xxx" and openai_key != "sk-your-actual-key-here" and not openai_key.startswith("sk-proj-06qEI"):
        try:
            ...
            print(f"[WARN] OpenAI generation failed: {e}. Falling back to mock...")

    # 3. Fallback mock RAG answer
    source_name = chunks[0]["metadata"]["source"] if chunks else "Nguon"
    mock_answer = f"Theo tai lieu {source_name} [{source_name.split('.')[0]}, 2024], hanh vi nay duoc de cap va nghiem cam."
    return {
        "answer": mock_answer,
        "sources": chunks,
        "retrieval_source": chunks[0].get("source", "hybrid") if chunks else "none"
